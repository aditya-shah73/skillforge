...
Bye.`}</CodeBlock>
      ...
      <section id="final">
        ...
        <Checkpoint moduleSlug="tool-use" id="final" title="Module complete" xp={40}>
          <p>
            You can now hand a model a typed view of your application&apos;s capabilities and let it orchestrate them. The next module — <Link href="/modules/streaming" className="text-indigo-600 hover:underline">Streaming with SSE</Link> — solves the latency problem you definitely noticed: every <code>.call()</code> blocks until the entire response is generated. Streaming gives you token-by-token output, which transforms the UX of any chat interface.
          </p>
        </Checkpoint>
      </section>

      <div className="mt-12 pt-8 border-t border-slate-200 dark:border-slate-800 flex justify-between text-sm">
        <Link href="/modules/spring-ai" className="text-indigo-600 hover:underline">← Module 10: Spring AI integration</Link>
        <Link href="/modules/streaming" className="text-indigo-600 hover:underline">Module 12: Streaming with SSE →</Link>
      </div>
    </article>
  );
}
