  ...
  ŷ  = the model's predicted probability for that class`}</CodeBlock>
        ...
        <Quiz
          ...
          options={[
            ...
            { label: "If you switched to MAE, Model A's error would be HIGHER relative to B — because MSE over-weights B's few huge errors", explanation: "Opposite! MAE relative to MSE would DEcrease B's penalty for those huge errors, because MSE squared them. So on MAE, B would look *worse relative to itself* than on MSE, meaning A would look comparatively better." },
            { label: "Under MAE, Model A would likely look BETTER than B, because B's huge errors get less of a disproportionate penalty under MAE but are still big", correct: true, explanation: "Right. MSE squares errors, punishing B's huge ones heavily. Under MAE, those huge errors only scale linearly — B's 'score' gets worse relative to a model like A with many small errors. Loss choice shapes which model 'wins'." },
